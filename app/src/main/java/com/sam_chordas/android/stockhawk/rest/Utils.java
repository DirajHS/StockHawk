...
public class Utils {
    ...
    public static ArrayList quoteJsonToContentVals(String JSON) {
        ...
        try {
            ...
            if (jsonObject != null && jsonObject.length() != 0) {
                jsonObject = jsonObject.getJSONObject(AppConstants.JSON_QUERY);
                int count = Integer.parseInt(jsonObject.getString(AppConstants.JSON_COUNT));
                if (count == 1) {
                    jsonObject = jsonObject.getJSONObject(AppConstants.JSON_RESULTS)
                            .getJSONObject(AppConstants.JSON_QUOTE);
                    ContentProviderOperation contentProviderOperation;
                    contentProviderOperation = buildBatchOperation(jsonObject);
                    if (contentProviderOperation != null)
                        batchOperations.add(contentProviderOperation);
                } else {
                    resultsArray = jsonObject.getJSONObject(AppConstants.JSON_RESULTS).getJSONArray(AppConstants.JSON_QUOTE);
    ...
    public static ContentProviderOperation buildBatchOperation(JSONObject jsonObject) {
        ...
        String changes = null;
        try {
            changes = jsonObject.getString(AppConstants.JSON_CHANGE);
        } catch (JSONException e) {
            e.printStackTrace();
        }
